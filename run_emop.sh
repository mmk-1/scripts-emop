...
do
    ...
    do
        ...
        do
            ...
            mvn clean test -Drat.skip # To make sure dependencies are installed
            rm -rf .starts/
            cp -r ../.starts/ ./.starts/
            ...
            mvn emop:$granularity -Drat.skip | tee >(grep -P "\[INFO\] Total time:\s+([\d.:]+\s\w+)" >> $LOG_PATH) >(grep -P "INFO: (.+: \d+)" >> $LOG_PATH) >(grep -P "\[INFO\] AffectedSpecs: (\d+)" >> $LOG_PATH) >(grep -P "\[INFO\] BUILD \w+" >> $LOG_PATH) >(grep -P "\[INFO\] \[eMOP Timer\] \w+" >> $LOG_PATH)
